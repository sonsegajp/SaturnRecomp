# Build the runtime with a reusable object cache and optional CPU/audio gameplay PGO.
# Example:
...
# Keep ObjectDir stable between generation and use; regenerate after core edits.
param(
...
$coreProfile=@()
if($Profile -ne 'off'){$coreProfile+="-fprofile-$Profile=$profileAbs"}
if($Profile -eq 'use'){$coreProfile+=@('-fprofile-correction','-Werror=missing-profile')}
$profiledCores=@('sh2_interp','m68k','scsp','scsp_dsp')
$sources=@('bus','scu_dsp','sh2_interp','bios','cdblock','vdp1','debugview','vdp2','m68k','m68k_bus','scsp','scsp_dsp','sound','png','smpc','boot','window','vulkan_renderer','runtime_settings','game_overlay','game_overlay_vk') | ForEach-Object {"runner/src/$_.c"}
...
foreach($source in $sources){
    ...
    $extra=@()
    if($name -in $profiledCores){$extra=$coreProfile}
...
$bootObjects=@($objects | Where-Object { [IO.Path]::GetFileName($_) -notin @('window.o','vulkan_renderer.o','game_overlay.o','game_overlay_vk.o') })
...
Write-Output "Built $BootOutput and $Output (CPU/audio profile: $Profile)"
exit 0
